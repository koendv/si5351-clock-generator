...
bool si5351mcu_not_ready() {  // return true if si5351 pll's have lost lock
...
#if 1
  if (status == -1) {
    USBSerial_println("i2c error");
  } else if (status & SI5351_STATUS_SYS_INIT) {
    USBSerial_println("SYS_INIT");
  } else if ((status & SI5351_STATUS_LOL_A) && clkOn[0]) {
    USBSerial_println("LOL_A");
  } else if ((status & SI5351_STATUS_LOL_B) && (clkOn[1] || clkOn[2])) {
    USBSerial_println("LOL_B");
  } else if (status & SI5351_STATUS_LOS_XTAL) {
    USBSerial_println("LOS_XTAL");
  }
#endif
  not_locked = (status == -1)                                                 // no i2c response
  ...
  return not_locked;                                                          // false if si5351 ok
}
